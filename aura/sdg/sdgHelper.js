({
    getNamespace: function (component) {
        ...
        var helper = this;
        try {
            this.callMethod(component, "c.GetNamespace", {}, { isStorable: true, isAbortable: true },
                    ...
                    if (results.isError) {
                        ...
                        if (results.SDGObject.SDGActions != null) {
                            ...
                            for (var i = 0; i < listsize; i++) {
                                ...
                                if (actiontype == 'List')
                                    hasListMenu = true;
                                if (actiontype == 'List Multi')
                                    hasMulti = true;
                                if (actiontype == 'Row Button')
                                    hasRowActions = true;
                                if (actiontype == 'Row') {
                                    hasRowMenu = true;
                                    hasRowActions = true;
    ...
    handleResults: function (component, resultsobj) {
        ...
        if (resultsobj.isError) {
                            ...
                            if (z == fieldparts.length - 1) {
                                ...
                                try {
                                    if (field.FieldType == 'CURRENCY') {
                                        ...
                                        FormattedValue = this.GetCaseInsensitiveAttr(datachunk, fieldparts[z] + 'Formatted');
                                    }
                                }
                                catch (getFormattedEx) {
                                    console.log('Unable to get formatted Currency');
                                }
            ...
            if (generateSummary) {
                for (var i = 0; i < fieldlistlength; i++) {
                    ...
                    if (field.FieldStyle && field.FieldStyle.startsWith("Summarize")) {
                        ...
                        for (var rowsummarizer = 0; rowsummarizer < payload.length; rowsummarizer++) {
            ...
            this.AddToLog(component, "Query returns: " + resultsobj.FullQueryCount + " rows");
            //current page:
            ...
            newTitle = newTitle + ' ' + ' (' + component.get("v.FullQueryCount") + ')';
    ...
    FireEvent: function (component, actionid, datarow) {
        ...
        if (evt != null) {
            ...
            if (datarow) {
                for (var fieldkey in datarow.data) {
                    ...
                    try {
                        ...
                        {
                            var newpath = field.Path.substring(0, field.Path.length - 4) + 'Id';

                            if (field.datachunkid)
                                payload = payload.replace("#" + newpath + "#", field.datachunkid);
            ...
            if (evt.Event != internalevent) {
                ...
                if (navEvt == null) {
                    this.showtoast('Error', 'Invalid event name - cannot identify event', component);
