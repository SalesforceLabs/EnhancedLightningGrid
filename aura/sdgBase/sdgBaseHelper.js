    ...
    handleError: function (component, ajaxresult) {
        ...
        if (errors) {
            ...
            if (errors[0] && errors[0].message) {
                var msg = 'An unhandled error occurred in the AJAX call: ' + errors[0].message;
    ...
    callMethod: function (cmp, methodname, params, config, onSuccess) {
        ...
        action.setCallback(this, function (actionResult) {
            ...
            if (state == 'SUCCESS') {
                ...
                if (errors) {
                    if (errors[0] && errors[0].message) {
                        var msg = "Error invoking " + methodname + ": " + errors[0].message;
                        self.AddToLog(cmp, msg);

                    }
                } else {
                    var msg = methodname + ': unknown error';
                    self.AddToLog(cmp, msg);
                }
            }
        })
